...
class _LedgerPageState extends ConsumerState<LedgerPage> {
  ...
  Future<void> _confirmAndDelete(
    ...
    // CRITICAL FIX: Check if the widget is still mounted before proceeding
    // If the LedgerPage itself was closed while the dialog was open,
    // we should not try to show a SnackBar on its context.
    if (!mounted) {
      return;
    }
    ...
    if (confirm) {
      try {
        ...
        // Check mounted again after async operation before showing SnackBar
        if (!mounted) {
          return;
        }
        ScaffoldMessenger.of(context).showSnackBar(
